...
ROS 2 Python package for monitoring a compression tester using red visual markers in a fixed camera view.

The node subscribes to a camera image, detects red marker dots with OpenCV, computes their bounding box, classifies the tester state from the marker height, and publishes an annotated image for the YOLO pipeline.

## Current Pipeline

```text
/image_raw
  -> compression_tester_monitor/red_dot_monitor
  -> /image_utm
  -> yolo_ros/yolo_node
  -> /yolo/dbg_image
```

The `yolo` alias is configured to launch YOLO with `input_image_topic:=/image_utm`.

## State Logic

The monitor finds red marker centers inside a fixed ROI and computes:

```text
x_min = minimum red marker x
x_max = maximum red marker x
y_min = minimum red marker y
y_max = maximum red marker y
span_y = y_max - y_min
```
...
| `/compression_tester/red_points` | `geometry_msgs/msg/PolygonStamped` | Detected marker centers as image-space points. |
...
| --- | ---: | --- |
| `input_image_topic` | `/image_raw` | Input camera image topic. |
...
| Parameter | Default | Description |
| --- | ---: | --- |
| `min_points` | `2` | Minimum number of red points required for a valid state. |
| `min_area` | `40.0` | Minimum red contour area. |
| `max_area` | `1500.0` | Maximum red contour area. |
| `min_circularity` | `0.4` | Minimum contour circularity. |
| `sat_min` | `80` | Minimum HSV saturation for red mask. |
| `val_min` | `80` | Minimum HSV value for red mask. |

Example threshold override:

```bash
ros2 launch compression_tester_monitor red_dot_monitor.launch.py working_height_threshold_px:=280.0
```
